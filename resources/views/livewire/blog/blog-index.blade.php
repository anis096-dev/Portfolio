<div wire:init="loadItems">
                ...
                <div class="flex flex-wrap items-center">
                    <div class="relative flex-row flex-1 w-full max-w-full px-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300"> {{ __('blogs') }}</h3>
                            </div>
                            <div class="ml-4">
                    ...
                    <div class="w-full overflow-x-auto">
                        <table class="w-full whitespace-no-wrap">
                            ...
                            <tr class="text-sm font-semibold text-gray-500 border-y ltr:text-left rtl:text-right dark:border-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700/30">
                                <th class="px-2 py-3 text-center">{{ __('Topic') }}</th>
                                <th class="px-2 py-3 text-center">{{ __('Title') }}</th>
                                <th class="px-2 py-3 text-center">{{ __('Desc') }}</th>
                                <th class="px-2 py-3 text-center">{{ __('Image') }}</th>
                            ...
                            @forelse($blogs as $blog)
                                <tr class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 hover:dark:text-gray-200 hover:bg-gray-100 hover:dark:bg-gray-700">
                                    ...
                                    <td class="px-2 py-3 text-sm text-center lowercase">
                                        {!! \Illuminate\Support\Str::limit($blog->desc, 30, '...') !!}
                                    </td>
                                    ...
                                    <td class="px-2 py-3 text-sm text-center">
                                        {{ $trashed ? $blog->deleted_at->diffForHumans() : $blog->created_at->diffForHumans() }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-between gap-1 text-sm text-center">
                                            ...
                                            @if($trashed)
                                                <x-button2 wire:click="selectedItem('restore',{{ $blog->id }})"
                                                                class="px-2">
                                                    <x-svg.svg-restore class="w-5 h-5"/>
                                                </x-button2>

                                                <x-button2
                                                        wire:click="selectedItem('forceDelete',{{ $blog->id }})"
                                                        class="px-2">
                                                    <x-svg.svg-force-delete class="w-5 h-5"/>
                                                </x-button2>
                                            @else
                                                <x-button2 wire:click="selectedItem('update',{{ $blog->id }})"
                                                                class="px-2">
                                                    <x-svg.svg-update class="w-5 h-5"/>
                                                </x-button2> 
                                                
                                                <x-button2 wire:click="selectedItem('show',{{ $blog->id }})"
                                                                class="px-2">
                                                    <x-svg.svg-show class="w-5 h-5"/>
                                                </x-button2>

                                                <x-button2 wire:click="selectedItem('delete',{{ $blog->id }})"
                                                                class="px-2">
    ...
    <livewire:blog.blog-update/>
    <livewire:blog.blog-show/>
    <livewire:blog.blog-delete/>
</div>
